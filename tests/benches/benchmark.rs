...
fn multisig_bench(c: &mut Criterion) {
    ...
    // 1. Benchmark: Committee creation (DKG/Keygen)
    group.bench_function("multisig committee create", |b| {
        b.iter(|| {
    ...
    // Prepare participants and committee for signing/verification benchmarks
    let participants = (0..SYSTEM_SIZE).map(|_| KeypairShare::default()).collect::<Vec<_>>();
    let mut committee = Committee::new();
    for share in participants.iter().map(|keypair| keypair.verifying_share.clone()) {
        committee.add_key(share.clone());
    ...
    // 2. Benchmark: Whole protocol (keygen, committee, sign, verify)
    group.bench_function("multisig whole protocol", |b| {
        b.iter(|| {
            // Key generation
            let participants = (0..SYSTEM_SIZE).map(|_| KeypairShare::default()).collect::<Vec<_>>();
            // Committee creation
    ...
    // 3. Benchmark: Signing round (from coordinator's message to aggregation)
    group.bench_function("multisig signing round", |b| {
        b.iter(|| {
            // Coordinator "sends" message to all signers (simulate by signing)
            let certificate = participants
                .iter()
                .map(|keypair| keypair.sign(message))
                .collect::<Vec<_>>();
            // Coordinator aggregates and verifies
            committee.verify(message, &certificate, THRESHOLD)
        });
    });

    // 4. Benchmark: Signing all shares
    group.bench_function("sign all", |b| {
        b.iter(|| {
            participants
                .iter()
                .map(|keypair| keypair.sign(message))
    ...
    // 5. Benchmark: Verify all signatures
    let certificate = participants
        .iter()
        .map(|keypair| keypair.sign(message))
        .collect::<Vec<_>>();
    group.bench_function("verify all", |b| {
        b.iter(|| {
            committee.verify(message, &certificate, THRESHOLD)
        });
    });

    // 6. Benchmark: Verify threshold signatures
    let certificate_threshold = participants
        .iter()
        .take(THRESHOLD)
        .map(|keypair| keypair.sign(message))
        .collect::<Vec<_>>();
    group.bench_function("verify threshold signatures", |b| {
        b.iter(|| {
            committee.verify(message, &certificate_threshold, THRESHOLD)
        });
    });
...
fn roast_bench(c: &mut Criterion) {
    ...
    group.bench_function("roast full protocol", |b| {
    ...
    // Measuring commitment exchange
    //group.bench_function(id, f)

    // --- NEW BENCHMARK: Measuring only Round 2 until aggregation ---
    group.bench_function("roast aggregate", |b| {
        b.iter_batched(
    ...
    group.bench_function("roast verify (from file)", |b| {
        b.iter_batched(
